 ...
 *
 * Демонстрація: форма з полями x, y → передача на сторінку обробки
 */
...
<div class="demo-card">
    <h2>Калькулятор</h2>
    <p style="color: var(--color-text-muted); margin-top: 0;">sin, cos, tg, my_tg, x^y, x!</p>

    <form method="post" action="task11_result.php" class="demo-form">
        <div class="form-row">
            <div>
                <label for="x">Значення X</label>
                <input type="number" id="x" name="x" step="any" value="<?= htmlspecialchars($_GET['x'] ?? '1.5') ?>" placeholder="Введіть X" required>
            </div>
            <div>
